...
# This program is designed to reproduce table 2 in the manuscript
# i.e., to analyze the two different types of testing outcomes
...
while(sim <=nIter){
	...
	########################################################################## model 2
	if(mod == 2){ # combine of function aepd and mix normal
	...
	res2<-try(Bayes.PP(Z=Z.dorf,Y=Y.dorf,Y.dorf_obs,D_1,mcx_1,mcw_1,
				   ...
				   est.error=TRUE,visual=FALSE),silent=TRUE)
	t2 <- proc.time()
	
	########################################## save results
	if(class(res2)!='try-error'){
		Theta <- rbind(res2$beta,res2$Se,res2$Sp,res2$tau_1,res2$tau_2,res2$phi_1,res2$phi_2)
	...
	if(class(res1)!='try-error'){
		Theta <- rbind(res1$beta,res1$Se,res1$Sp,res1$tau_1,res1$tau_2,res1$phi_1,res1$phi_2)
		Theta.mean <- apply(Theta,1,mean)
		Theta.med <- apply(Theta,1,median)
		Theta.sd <- apply(Theta,1,sd)
		Theta.bd <- HPDinterval(as.mcmc(t(Theta)))
